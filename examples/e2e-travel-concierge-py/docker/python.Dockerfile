...
# Nothing outside this repo is needed -- helixid-sdk-py is installed straight
# from the public helixid/helix-sdk-py repo, so there's no sibling checkout.
#
...
# gcc for any sdist that needs compiling; git resolves the pip git+ spec below.
...
# helix-sdk-py from its public repo. The mcp-middleware extra pulls in the
# official MCP SDK that the mcp_server package uses for the wire protocol.
